  ...
  const downloadBatch = async (format) => {
    ...
    try {
      ...
      if (format === 'pdf') {
        ...
        header.innerHTML = `
          <h1 style="color: #333; margin-bottom: 10px;">BULK QR CODES REPORT</h1>
        ...
        qrWithBase64.forEach((qr, index) => {
          ...
          qrSection.innerHTML = `
            <div style="display: flex; align-items: flex-start; gap: 20px;">
              <div style="flex: 1;">
                <h3 style="color: #333; margin-bottom: 10px;">${index + 1}. ${qr.qrCode.title}</h3>
              ...
              <div style="text-align: center;">
                <img src="${qr.qrBase64}" alt="QR Code" style="width: 100px; height: 100px; border: 1px solid #ddd;">
              </div>
            </div>
        ...
        const opt = {
          margin: 10,
          filename: `bulk-qr-codes-${Date.now()}.pdf`,
        ...
        let content = `BULK QR CODES DOCUMENT\n`;
        ...
        link.href = url;
        link.download = `bulk-qr-codes-${Date.now()}.doc`;
        ...
        successfulQrs.forEach((qr, index) => {
      const link = document.createElement('a');
          link.href = qr.qrCode.qrImage.url;
          link.download = `${qr.qrCode.title || `qr-code-${index + 1}`}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
        });
